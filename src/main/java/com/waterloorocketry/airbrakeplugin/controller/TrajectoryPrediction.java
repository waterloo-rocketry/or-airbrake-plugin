...
public class TrajectoryPrediction {
     ...
     * */
    public static double get_max_altitude(double extRef, Controller.RocketState rocketState) {
        double vX = Math.sqrt(rocketState.velocityX * rocketState.velocityX + rocketState.velocityY * rocketState.velocityY);
        return ProcessorCalculations.getMaxAltitude((float) extRef, (float) rocketState.velocityZ, (float) vX, (float) rocketState.positionZ); //z displacement, z velocity, ...; +Z is "up" in OR
    }
}
